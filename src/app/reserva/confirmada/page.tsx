...
async function ConfirmadaContent({ code }: { code: string }) {
  ...
  return (
    <div className="max-w-md mx-auto text-center">
      ...
      <h1 className="text-3xl font-black text-gray-900 mb-2">{t.confirmed}</h1>
      <p className="text-gray-500 mb-8">{t.confirmationEmail} <strong>{booking.customer_email}</strong></p>
      ...
      <div className="bg-gray-50 rounded-3xl p-6 text-left space-y-3 mb-8">
        ...
        <div className="flex justify-between text-sm">
          ...
          <span className="font-semibold text-gray-800">
            {new Date(booking.booking_date + 'T00:00:00').toLocaleDateString(lang === 'en' ? 'en' : lang === 'de' ? 'de' : lang === 'fr' ? 'fr' : 'es', { weekday: 'long', day: 'numeric', month: 'long' })}
          </span>
        </div>
        <div className="flex justify-between text-sm">
      ...
      {wa && (
        <a href={`https://wa.me/${wa}?text=Hola! Tengo una reserva confirmada, código ${booking.confirmation_code}`}
          ...
          {t.contactOperator} {exp?.operators?.name}
        </a>
      )}
    </div>
  )
